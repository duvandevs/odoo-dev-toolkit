...
export class QuickUpdaterRWWidget extends Component {
    static template = "quick_updater_rw.QuickUpdaterRWWidget";
    static props = {};
    ...
    loadFavoriteIds() {
        ...
        const stored = localStorage.getItem('quick_updater_rw_favorites');
        if (stored) {
            try {
    ...
    saveFavorites() {
        ...
        localStorage.setItem('quick_updater_rw_favorites', JSON.stringify(this.state.favoriteIds));
    ...
    async loadFavoriteModules() {
        ...
        try {
            const result = await this.rpc("/quick_updater_rw/get_favorite_modules", {
    ...
    async searchModules(searchTerm) {
        ...
        try {
            const result = await this.rpc("/quick_updater_rw/search_module", {
    ...
    async updateModule(moduleId, moduleName) {
        ...
        try {
            const result = await this.rpc("/quick_updater_rw/update_module", {
                module_id: moduleId,
            });
...
QuickUpdaterRWWidget.template = "quick_updater_rw.QuickUpdaterRWWidget";

registry.category("systray").add(
    "QuickUpdaterRWWidget",
    {
        Component: QuickUpdaterRWWidget,
    },
    { sequence: 10 }
);
